...
if ( ! defined( 'ABSPATH' ) ) {
	wp_die( __( "Cannot access file directly", 'howsu-demo' ) );
}

if ( ! class_exists( 'Howsu_Demo\Utilities' ) ) {
	...
	class Utilities {
		...
		private function __construct() {
			...
			if ( empty( self::$plugin_slug ) ) {
				self::$plugin_slug = apply_filters( 'e20r-licensing-text-domain', 'howsu-demo' );
			}
		...
		public function display_messages( $source = 'default' ) {
			...
			if ( ! empty( $this->msg ) && ! empty( $this->msgt ) ) {
				...
				$this->log( "Have " . count( $this->msg ) . " possible admin message(s) for source {$source} (to display" );
    
				foreach ( $this->msg as $key => $notice ) {
				    if ( $source === 'default' || $this->msgt[$key] === $source ) { ?>
