    ...
    <?php while ($this->have_groups()) : $this->the_group(); ?>
        <?php while ($this->have_fields()) {
            ...
            <?php if ($this->group->name == "main") { ?>
                <table>
                    <?php if($this->field->name != 'textitid' && $this->field->name != 'mailing_list') { ?>
                    <tr class="<?php echo Participants_Db::$css_prefix . $this->field->name . ' ' . $empty_class; ?>">
                        ...
                        <td width="20%">
                            <?php if($this->field->name != 'user_id') { ?>
                            <div class="<?php echo $this->field->name; ?>_edit">
                                <a id="edit" href="" name="<?php echo $this->field->name; ?>"><i class="fa fa-pencil-square-o fa-2x"></i></a>
                            </div>
                            <div class="edit <?php echo $this->field->name; ?>_save">
                                <a type="submit" id="save" href="" name="<?php echo $this->field->name; ?>"><i class="fa fa-floppy-o fa-2x"></i></a>
                            </div>
                            <?php } ?>
                        </td>
                    </tr>
